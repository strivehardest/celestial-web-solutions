...
function drawLetterhead(doc, fonts, agreementId, logoPath, pageNumber, pageCount, createdAt) {
  ...
  doc
    ...
    .text(COMPANY.name.toUpperCase(), textLeft, 18, {
      width: pageWidth - textLeft - MARGIN_X - 155,
      lineBreak: false,
  ...
  // Right-side document identity card
  const idBoxW = 148;
  const idBoxX = pageWidth - MARGIN_X - idBoxW;
  doc.roundedRect(idBoxX, 16, idBoxW, 48, 5).strokeColor(RULE_STRONG).lineWidth(1).stroke();
  doc
    .fillColor(ORANGE)
    .font(fonts.bold)
    .fontSize(7)
    .text('CLIENT PROJECT AGREEMENT', idBoxX + 8, 22, {
      width: idBoxW - 16,
      align: 'center',
      lineBreak: false,
  ...
  doc
    ...
    .font(fonts.regular)
    .fontSize(7)
    .text(`Issued ${formatDate(createdAt)}`, idBoxX + 8, 48, {
      width: idBoxW - 16,
      align: 'center',
      lineBreak: false,
    });
