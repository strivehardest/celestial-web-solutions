...
async function sendServiceRequestEmails({
  ...
  const { createAgreementId, isValidAgreementId } = require('./agreementHelpers');
  const agreementId = isValidAgreementId(providedAgreementId)
    ? String(providedAgreementId).trim().toUpperCase()
    : createAgreementId(fullName);
  const createdAt = providedCreatedAt || new Date().toISOString();
  if (providedAgreementId && !isValidAgreementId(providedAgreementId)) {
    console.warn(
      `[agreement] ignoring invalid providedAgreementId=${providedAgreementId}; minted ${agreementId}`
    );
  }
  console.log(`[agreement] service request using Agreement ID ${agreementId}`);
  const fields = {
    Name: fullName,
    Email: email,
    Phone: phone || 'Not provided',
  ...
  try {
    ...
    if (pdfResult.agreement?.agreementId !== agreementId) {
      console.error(
        `[agreement] email PDF id mismatch: expected=${agreementId} got=${pdfResult.agreement?.agreementId}`
      );
    } else {
      console.log(`[agreement] email PDF attached with Agreement ID ${agreementId}`);
    }
    pdfAttachment = {
