...
const TOPICS = [
  ...
  },
  {
    ...
    weight: 0.7,
    answer: () =>
      `You're welcome. Whenever you're ready to start, message us on [WhatsApp](https://wa.me/233530505031), ${link('schedule a free call', '/schedule-a-call')}, or explore the ${link('pricing page', '/pricing')}. Is there anything else you'd like to know?`,
  },
  {
    id: 'about',
  ...
  },
  {
    ...
    answer: () => {
      ...
      return `We have launched **${PORTFOLIO.length} portfolio projects** for clients in Ghana, the United States and beyond. Here is the full catalogue:\n\n${catalogue}\n\nAsk for a deep dive on any project by name (for example "Tell me about Ghanas Event" or "Deep dive into Finance Tracker") and I will cover the overview, client, timeline, full tech stack and every feature. Or browse the ${link('portfolio', '/portfolio')}.`;
    },
  },
  {
    id: 'courses',
...
export function answerLocally(question, { language = 'en' } = {}) {
  ...
  if (bestProject && bestProject.score >= 4 && !isCatalogueAsk) {
    ...
    if (secondProject && secondProject.score >= bestProject.score * 0.85 && secondProject.score >= 5) {
      ...
      reply += `\n\nWant the same depth on another project? Ask for it by name, or browse the ${link('full portfolio', '/portfolio')}.`;
    }
  ...
  if (!ranked.length) {
    ...
    if (bestProject && bestProject.score >= 2) {
