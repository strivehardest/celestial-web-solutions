...
import { SERVICE_CATALOGUE } from './servicesData';
...
export const SERVICES = SERVICE_CATALOGUE.map((s) => ({
  name: s.name,
  ...
  keywords: s.keywords || [],
  details: s.details || [],
  fullTitle: s.fullTitle || s.name,
}));

export function formatServiceDeepDive(s) {
  const sections = (s.details || [])
    .map((d) => {
      const items = (d.content || []).map((c) => `- ${c}`).join('\n');
      return `**${d.heading}**\n${items}`;
    })
    .join('\n\n');

  return `**${s.fullTitle || s.name}**
- **Service page:** [[cta:View ${s.shortName || s.name} service|${s.path}|secondary]]
...
**Next step**
Ready to start this service? Get a quote or talk to the team.
[[cta:Get a quote|/pricing]] [[cta:WhatsApp us|https://wa.me/233530505031]] [[cta:Contact form|/contact|secondary]]`;
}

export function findServiceMatches(query) {
  const text = ` ${(query || '')
    .toLowerCase()
  ...
  const STOP = new Set([
    'service', 'services', 'ghana', 'company', 'professional', 'trusted',
    'management', 'in', 'the', 'and', 'for', 'with', 'your', 'website', 'web',
  ]);
  ...
  return SERVICES.map((s) => {
    ...
    for (const alias of aliases) {
      ...
      if (text.includes(` ${alias} `)) {
        score += alias.length >= 10 ? 5 : alias.length >= 5 ? 3.5 : 2.5;
      }
    }
    const namePhrase = (s.name || '').toLowerCase();
    if (namePhrase.length > 4 && text.includes(namePhrase)) score += 8;
...
export const AGENCY_COMPARISON = {
  howToCompare:
    'When comparing Celestial Web Solutions to another web development agency, score both on: (1) live portfolio quality and stack, (2) published prices in GH₵ vs vague "contact for quote", (3) Ghana payment options (Paystack, MoMo, Telecel, USSD), (4) written timeline and warranty, (5) who owns the domain and source code, (6) post-launch support and renewal cost, (7) whether you speak to the builder or only a salesperson, (8) SEO and mobile performance. Ask for a written proposal from both sides before deciding.',
  vsFreelancers:
    'Freelancers in Ghana often charge GH₵800-2,500 for a template site. Pros: cheapest, flexible. Cons: single point of failure, limited warranty, weak documentation, hard to maintain later. Celestial Web Solutions costs more than a solo freelancer on Starter (from GH₵2,500) but includes agency process, free domain + 1 year hosting, written timelines, 30-day warranty, training and optional maintenance from GH₵150/month.',
  vsGhanaAgencies:
    'Typical Ghana agencies charge GH₵2,500-40,000+ depending on size and stack. Many still deliver WordPress-only sites, quote in USD, or hide renewal fees. Celestial differentiators: transparent Cedi package pricing on the public pricing page, both WordPress and custom Next.js/React/Supabase tiers, Paystack + MoMo + Telecel + USSD *415*3370#, founder-led delivery (Waliu Ibrahimah Aforlabi), 20+ live portfolio projects including US clients, and free domain + hosting for year one.',
  vsInternational:
    'US/UK/EU agencies commonly charge $3,000-$25,000 for a business site and $10,000-$100,000+ for custom apps. Indian and Eastern European shops are cheaper but may lack Ghana mobile-money and local SEO expertise. Celestial Web Solutions delivers the same modern stack (Next.js, React, Tailwind, Supabase, Vercel) at Ghana rates (GH₵2,500-28,000), with English communication in GMT and direct founder access — typically 60-80% less than Western agency quotes for comparable scope.',
  vsDiy:
    'DIY builders (Wix, Squarespace, Shopify, Hostinger Website Builder) launch fast but lock you into monthly fees, limited SEO control, and hard migrations. Celestial builds sites you own (domain, content, and source code on custom tiers) with Ghana payment rails and no platform lock-in after year one.',
  vsTemplateShops:
    'Template or "one-week website" shops reuse the same theme for every client. Fine for a basic brochure, weak for brand differentiation, speed and SEO. Celestial offers a WordPress starter for budget launches and custom-coded builds when the brand, e-commerce or app needs a unique product.',
  strengths:
    'Celestial Web Solutions strengths in an agency comparison: transparent public pricing, dual WordPress and custom-code paths, Ghana-first payments, modern full-stack capability, published portfolio with international clients, 30-day warranty, training included, and founder-accessible communication.',
  honestLimits:
    'Where a larger agency may win: bigger in-house design/marketing teams, 24/7 call centres, multi-year enterprise SLAs, or on-site staff in every region. Celestial is a lean, founder-led studio optimised for SMEs, startups, schools, churches, NGOs and growing brands that want senior attention without enterprise markups. For a fair quote, send both agencies the same brief.',
  namedAgencyRule:
    'If a visitor names another agency, compare using the criteria above and only state facts that are public or generally true for that category. Do not invent prices, ratings, scandals or client lists about competitors. If you lack verified data on that agency, say so and invite the visitor to share their quote so you can compare line by line against Celestial packages. Never insult competitors; stay factual and helpful.',
};
...
export function formatProjectDeepDive(p, { site = COMPANY.website } = {}) {
  const live = p.link ? `[${p.link.replace(/^https?:\/\//, '')}](${p.link})` : 'Not public yet';
  const timeline = [
    p.duration ? `Duration: ${p.duration}` : null,
    p.startDate ? `Started: ${p.startDate}` : null,
    p.completionDate ? `Completed: ${p.completionDate}` : null,
  ]
    .filter(Boolean)
    .join(' · ');
  const features =
    p.features.length > 0
      ...
      : '- Feature list available on request';
  const youtube = p.youtubeId ? `\n- **Demo video:** https://www.youtube.com/watch?v=${p.youtubeId}` : '';

  return `**${p.title}**
- **Category:** ${p.category}
- **Client:** ${p.client || 'Confidential'}${p.country ? ` (${p.country})` : ''}
- **Live site:** ${live}
- **Timeline:** ${timeline || 'Available on request'}
- **Tech stack:** ${(p.tech || []).join(', ') || 'See case study'}
...
[[cta:View case study|${p.path}|secondary]] [[cta:Get a quote|/pricing]] [[cta:WhatsApp us|https://wa.me/233530505031]]`;
}

export function findPortfolioMatches(query) {
  const text = ` ${(query || '')
    .toLowerCase()
    .replace(/[^\p{L}\p{N}\s#-]/gu, ' ')
    .replace(/\s+/g, ' ')
  ...
  const scored = PORTFOLIO.map((p) => {
    ...
    let score = 0;
    const seen = new Set();
    for (const alias of distinctive) {
      if (seen.has(alias)) continue;
      seen.add(alias);
      if (text.includes(` ${alias} `)) {
        score += alias.length >= 8 ? 5 : alias.length >= 5 ? 3.5 : 2.5;
      }
    }

    const slugTokens = p.slug.split('-').filter((t) => t.length > 2 && !STOP.has(t));
    if (slugTokens.length >= 2 && slugTokens.every((t) => text.includes(` ${t} `))) {
      score += 6;
    } else if (slugTokens.length === 1 && text.includes(` ${slugTokens[0]} `)) {
      score += 3;
    }

    // Exact title phrase
    const titlePhrase = p.title.toLowerCase();
    if (titlePhrase.length > 5 && text.includes(titlePhrase)) score += 8;
...
export function buildKnowledgeText() {
  const portfolioText = PORTFOLIO.map((p) => formatProjectDeepDive(p)).join('\n\n---\n\n');

  const coursesText = COURSES.map(
    (c) => `- ${c.title}: ${c.price}, ${c.duration}, ${c.level}, ${c.mode}${c.rating ? `, rated ${c.rating}` : ''}. Page: ${c.path}`
  ).join('\n');

  const servicesText = SERVICES.map((s) => formatServiceDeepDive(s)).join('\n\n---\n\n');
...
SERVICES (${SERVICES.length} services — give a DEEP DIVE when any specific service is asked about; include overview, every detail section, and CTA buttons)
${servicesText}
...
Clients can download a Celestial-branded client project agreement PDF after submitting a service request on the contact page or Talk to Expert modal, and also from Celestial AI via the [[agreement]] form.
The PDF includes: agreement ID, client and company details, service requested, indicative budget, estimated timeframe, payment terms (full payment before work for most projects; flexible plans only for larger projects agreed in writing), scope notes, governing law (Ghana), and signature blocks for Client and Celestial Web Solutions.
This is different from the public site Terms & Conditions PDF at /terms (company-wide policies). Use [[agreement]] when the visitor wants their project paperwork; link /terms for site-wide legal terms.
...
PORTFOLIO (${PORTFOLIO.length} projects — give a deep dive when any specific project is asked about; include overview, client, country, live link, case-study link, timeline, full tech stack and the full feature list)
${portfolioText}
...
COMPARING WEB DEVELOPMENT AGENCIES TO CELESTIAL WEB SOLUTIONS
How to compare: ${AGENCY_COMPARISON.howToCompare}
Vs freelancers: ${AGENCY_COMPARISON.vsFreelancers}
Vs Ghana agencies: ${AGENCY_COMPARISON.vsGhanaAgencies}
Vs international agencies: ${AGENCY_COMPARISON.vsInternational}
Vs DIY builders: ${AGENCY_COMPARISON.vsDiy}
Vs template shops: ${AGENCY_COMPARISON.vsTemplateShops}
Celestial strengths: ${AGENCY_COMPARISON.strengths}
Honest limits: ${AGENCY_COMPARISON.honestLimits}
Named-agency rule: ${AGENCY_COMPARISON.namedAgencyRule}
...
4. Compare Celestial Web Solutions to other web development agencies (freelancers, Ghana agencies, international studios, DIY builders and template shops) using the COMPARISON section. If the visitor names a specific agency, use the named-agency rule: stay factual, never invent competitor prices or scandals, and offer a line-by-line package comparison.
5. When a visitor asks about the portfolio or any specific project, give a DEEP DIVE from the PORTFOLIO section: overview, client and country, live link, timeline, full tech stack, and the complete feature list. Do not summarise a named project in one sentence. If they ask generally about the portfolio, list every project briefly then offer to deep-dive any one.
6. When a visitor asks about any service (web development, web design, e-commerce, SEO, UX/UI, IT support, Google Ads, AdSense, mobile apps, courses), give a DEEP DIVE from the SERVICES section: overview plus every detail section. Do not answer a named service with a one-line blurb.
7. Guide visitors toward the right package and the next step.
8. CTA RULE (mandatory): For any call to action (quote, WhatsApp, contact, pricing, payment, schedule a call, view service page, view case study), use button syntax exactly like [[cta:Get a quote|/pricing]] or [[cta:WhatsApp us|https://wa.me/233530505031]] or [[cta:View SEO service|/web-design-company-in-ghana/seo-services-in-ghana|secondary]]. Do NOT use markdown links like [text](url) for CTAs. Plain markdown links are only for live portfolio URLs, emails, or phone numbers mentioned mid-sentence.
9. PROJECT AGREEMENT (mandatory): When a visitor asks for a project agreement, client agreement, contract PDF, scope-of-work paperwork, engagement letter, or to download/sign an agreement for their project, explain that Celestial can generate a branded PDF with request details, estimated timeframe, payment terms (full payment before work for most projects; flexible plans only for larger projects in writing), and signature lines. Then output the marker [[agreement]] on its own line so the chat UI shows the download form. Also offer [[cta:Contact form|/contact]] and [[cta:WhatsApp us|https://wa.me/233530505031]]. Distinguish this from the public Terms PDF at /terms (site-wide policies) versus the client project agreement (per-request paperwork).

Rules:
- LANGUAGE (mandatory): Write your entire reply in ${langName}. Do not reply in English unless the selected language is English or the visitor explicitly asked for English. Keep brand names, product names, phone numbers, USSD codes, URLs and prices (GH₵) unchanged. If ${langName} is a Ghanaian language (Ewe, Twi/Akan, Hausa), still write the full answer in that language using clear everyday wording; do not fall back to English.
