...
export function formatProjectDeepDive(p, { site = COMPANY.website } = {}) {
  ...
  const caseStudy = `[Case study](${site}${p.path})`;
  const timeline = [
...
**Why it matters**
This project shows Celestial Web Solutions' ability to deliver ${p.category} work for ${
    p.country || 'clients'
  } using ${(p.tech || []).slice(0, 4).join(', ') || 'modern web technologies'}. Ask for a similar build or browse more work in the [portfolio](${site}/portfolio).`;
}
...
5. When a visitor asks about the portfolio or any specific project, give a DEEP DIVE from the PORTFOLIO section: overview, client and country, live link, case-study link, timeline, full tech stack, and the complete feature list. Do not summarise a named project in one sentence. If they ask generally about the portfolio, list every project briefly then offer to deep-dive any one.
6. Guide visitors toward the right package and the next step (WhatsApp +233 53 050 5031, call +233 24 567 1832, the contact page, or the pricing page).
