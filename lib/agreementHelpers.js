...
/**
 * Accepts current (4-char slug) and older (up to 6–12 char slug) CWS ids.
 * Format: CWS-YYYYMMDD-SLUG-RAND4
 */
function isValidAgreementId(value) {
  return /^CWS-\d{8}-[A-Z0-9]{1,12}-[A-Z0-9]{4}$/i.test(String(value || '').trim());
}
 ...
 *   allowCreate (default true): mint a new CWS id when none/invalid is provided.
 *   Set false for download paths so a missing id cannot silently mint a second code.
 */
function buildAgreementData(raw = {}, options = {}) {
  ...
  const providedId = String(raw.agreementId || '').trim();
  let agreementId;
  if (isValidAgreementId(providedId)) {
    agreementId = providedId.toUpperCase();
  } else if (allowCreate) {
    agreementId = createAgreementId(fullName);
  } else {
    const err = new Error(
      'A valid agreementId from the service request is required to download this PDF.'
    );
    err.code = 'MISSING_AGREEMENT_ID';
    throw err;
  }
