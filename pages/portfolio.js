...
// ReviewBadges
// ─────────────────────────────────────────────────────────────────────────────
const STARS_YELLOW = Array.from({ length: 5 });

const StarRow = ({ color = "text-yellow-400", opacity = 1 }) => (
  <div className="flex items-center gap-0.5" style={{ opacity }}>
    {STARS_YELLOW.map((_, i) => (
      <svg key={i} className={`w-3 h-3 ${color}`} viewBox="0 0 24 24" fill="currentColor">
    ...
    <div className="max-w-4xl mx-auto">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-px bg-white/10 rounded-2xl overflow-hidden border border-white/15 backdrop-blur-md shadow-xl shadow-black/20">
        ...
        >
          ...
          <StarRow color="text-red-400" opacity={0.4} />
...
// CategoryFilters
// ─────────────────────────────────────────────────────────────────────────────
const CategoryFilters = ({ filter, onFilterChange }) => (
...
const HappyClients = () => (
  <section className="py-12 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 rounded-3xl mt-24 mb-12">
    ...
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 md:gap-8 justify-items-center items-center px-4">
      {HAPPY_CLIENTS.map((client, idx) => {
        const isSmall = client.name === "Elolo Agbleke" || client.name === "Tru Seeds Africa";
        return (
  ...
  >
    ...
    <div className="relative z-10 text-center px-4">
      ...
      <p
        ...
        style={{ fontFamily: "Albert Sans, sans-serif" }}
      >
        Let's create something amazing together. Whether it's a website, e-commerce platform, or
        custom web application, we're here to bring your vision to life.
      </p>
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 py-32">
            ...
            >
              ...
              <p
                ...
                style={{ fontFamily: "Albert Sans, sans-serif" }}
              >
                From sleek corporate websites to powerful e-commerce platforms — explore how we've
                helped businesses across Ghana, the U.S., and beyond establish a powerful online
                presence.
              </p>
            ...
            >
              {filteredProjects.map((project, index) => (
                <PortfolioCard
                  key={project.slug}
                  project={project}
                  image={PORTFOLIO_IMAGES[project.slug] ?? PLACEHOLDER_IMAGE}
                  index={index}
                />
              ))}
            </motion.div>
